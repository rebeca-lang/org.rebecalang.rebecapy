...
class ErrorCode(enum.Enum):			# Win32 Error codes
	ERROR_CONTINUE			= 1246
...
# Note: Imported from utilities library in 'cospy'
class TreeFindCtxt:
...
class TreeSearchCtxt:
	def init__(self, name:str, flag=0):
		""" Tree search context
		Arguments
			name -- Name of the tree
			flag -- Flags for the search
...
class Tree(Container, list):
	...
	def count(self):
		...
		"""
		result			= TreeSearchCtxt()
		result.count	= 0

		self.traverse( Tree.__count_node, result )
		return result.count+self._root.size+1
	...
	def match( self, name:str, wildcard=False ):
		...
		node	= self._root.match( name, flag )
		if len(node)==0:
			return ''
		
		return ','.join( [n.path for n in node] )
	...
	def find( self, path:str ):
		...
		return True if self._root.find(path) is not None else False
	...
	def exists( self, path:str ):
		...
		Arguments
			path -- Path of the node
		"""
		return True if self.find_node(path) is not None else False
	...
	@staticmethod
	def __count_node( result, node ):
		""" Internal callback function to handle dumping a tree node
		Arguments
			result -- Context argument passed to the callback
			node -- Node to process
		"""
		result.count += node.size
		return ErrorCode.ERROR_CONTINUE
...
if __name__ == "__main__":
	t = Tree()
	print( f'Size     = {t.count()}' )		# Only root
	n = t.add( '/Child1/GrandChild1.1/XXX', 'XXX' )
	t.add( '/Child2/GrandChild2.1/XXX2', 'XXX2' )
	t.add( '/Child2/GrandChild2.1/Y', 'Y' )
	t.add( '/Child2/GrandChild2.2', 'Z' )

	print( '\n=========== Tree Dump ===========' )
	print(t)
	print( '=================================\n' )

	print( f'Size     = {t.count()}' )		# Root and all the children

	print( f'Exist     = {t.exists('/Child2/GrandChild2.1/Y')}' )
	print( f'NotExist     = {t.exists('/Child2/GrandChild2.1/Not')}' )
	print( f'Path(Y)  = {t.match("Y")}' )
	print( f'Path(X*)  = {t.match("X*",True)}' )
